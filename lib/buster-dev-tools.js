...
var dependencyGraph = (function() {
    var externalProjects = {};  // keep a hash of ever encountered externalProjects, without polluting namespace
    return function(projects, project, done) {
        function initVertex(projectName, version) {
            ...
            if (isInternalProject(projects, projectName)) {
                ...
                if (!(project = externalProjects[projectName])) {
                    project = externalProjects[projectName] = { name: projectName, isInternal: false, version: version };
                }
            }
            if (!project.vertex) {
                project.vertex = {
        ...
        ["dependencies",
         ...
         "optionalDependencies"
            ...
            if (!(depType in pkg)) return;
            for (var childName in pkg[depType]) {
                var child = initVertex(childName, pkg[depType][childName]);
                console.log(depType + ': ' + v.dotId + ' -> ' + child.dotId);
            }
        });
        done();
    };

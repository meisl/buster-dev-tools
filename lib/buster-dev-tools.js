...
function doInParallel(projects, operations, maxPar) {
    projects = projects.map(getProject);
    ...
    for (var i=0; i<projects.length; i++) {
        queue.push(
            (function(project) {
                return function() {
                    var nextJob; // let's just not call it "task" as in the outer scope...
                    working++;
                    //console.log(working + " parallel");
                    op(projects, project, function() {
                        working--;
                        //console.log(working + " parallel");
                        util.print('.');
                        if ((working < maxPar) && (nextJob = queue.shift())) {
                            nextJob();
                        }
                    });
                };
    ...
    maxPar = (maxPar >= 1) ? maxPar : 10;   //  os.cpus().length; //  1;    //  
    while ((working < maxPar) && (task = queue.shift())) {
        task();
    }
}
    ...
    ["dependencies",
     "devDependencies"].forEach(function (prop) {
        ...
        for (var dep in pkg[prop]) {
            ...
            if ((process.platform == "win32") && (depWithVer == "faye@~0.7")) { // v~0.7 not installable on Win, just leave it out for now 
